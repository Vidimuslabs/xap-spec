...
verifier can check it, and the two are routinely confused — a schema that
carries a field implies someone evaluates it. For every field below, nobody
does, and the honest thing is to say so here rather than let the schema imply
otherwise. An implementation **MUST NOT** report any of these as verified.

They divide by *why* they are unverifiable, and the division matters, because
only the first group is unverifiable in principle.

**Requires state the verifier deliberately does not have.** Verification is
defined to succeed from the receipt, reproduced inputs and public anchors alone,
with no enforcement-point state (¶0017). Replay detection contradicts that
directly: deciding whether a nonce has been seen before requires remembering
every nonce seen before.

- `replay.nonce` (MAT field 138) and `session_id` (commitment, ¶0095B). A
  verifier can confirm a nonce is **present** — its absence means there is no
  replay protection at all, which is a different and worse thing than replay
  protection this verifier cannot evaluate — and the reference verifier requires
  presence. Whether it is *fresh* is a question for a party holding a seen-set.

**Reproducible only with inputs the receipt does not disclose.**

- `resource_state_digest` (¶0054) is a digest over the resource keys the request
  named. The receipt does not record which keys those were, so a verifier
  holding the reproduced context still cannot select the same subset. It is
  meaningful to the enforcement point confirming a speculative evaluation
  against its own earlier read, and not to anyone else.
- `policy_digest` (¶0076) is a digest over the enforcement point's compiled
  constraint form. Compilation is an implementation's internal representation,
  not a protocol artifact, so two conforming engines may compile the same
  constraints differently and both be right.

**A judgement, not a computation.** These have no correct value to recompute
against.

- `trust_vector` (MAT field 128) is the issuer's assessment of the subject.
- `scope.policy` is an opaque policy expression the engine evaluates; the
  protocol assigns it no structure to check.

**Carried as a label, with nothing to bind it to.**

- `enforcement_point` names the emitting enforcement point as a string. The
  receipt carries no key identifier for it, so unlike the MAT's issuer identity —
  which step 4 binds to the key that signed — there is nothing to compare it
  against. It is a label for operators, not evidence.

Two fields formerly in this list are now verified, and are named here so the
list is not read as fixed: the MAT's `issuer.kid` is bound to the verifying key
(step 4), and a commitment's `agent_identity` is bound to the machine identity
its governing MAT authorizes (step 8). The test for whether a field belongs
here is whether an independent party, given the artifacts and the public
anchors, can reach a conclusion about it — not whether it is currently checked.
