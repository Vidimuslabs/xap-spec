...
8. If a commitment is supplied: validate its signature, verify its binding to the
   ...
   no such window. Where the commitment discloses an `agent_identity`, confirm it
   is well formed before binding it to anything: its `kind` **MUST** be a
   recognized discriminant and **MUST** agree with the material it carries
   (field 122), so a `kind` of `"attestation"` over a bare public key, or a
   `kind` outside the registered set, is rejected — a `kind` a relying party
   reads to learn how a machine proved itself is a claim, and a claim the
   material does not support is not merely unverified. Report **not performed**
   where the commitment discloses no identity. Then confirm the commitment's
   `agent_identity` is the machine identity the governing MAT authorizes
   (field 122): the MAT names who the authority is FOR and the commitment names
   who it was generated BY, and a disagreement means the commitment is not the
   one that artifact governs. Report **not performed** where either side
...
| `agent_identity_wellformed` | commitment | 8 | ✓ (commitment discloses no agent identity) | the commitment's agent identity kind is recognized and agrees with the material it carries (field 122) |
