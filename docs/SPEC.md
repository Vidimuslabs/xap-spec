...
6. If the reproduced context is supplied: recompute the context digest and
   ...
   of exactly those keys (¶0054) — a digest carried without its keys is
   unreproducible and reports **not performed**; recompute each recorded constraint outcome and compare; check
...
8. If a commitment is supplied: validate its signature, verify its binding to the
   ...
   Report **not performed** where either side discloses no identity. Where the
   caller supplies a replay seen-set, confirm the commitment's `session_id` has
   not been accepted before (¶0095B).
...
- `replay.nonce` and `session_id` are checked against a **verifier-supplied**
  seen-set. ¶0017 forbids relying on ENFORCEMENT-POINT state; it says nothing
  about the verifier's own, and a relying party that remembers what it has
  accepted learns nothing from the issuer by doing so. A verifier that keeps no
  seen-set reports these NOT PERFORMED. See step 4 and step 8.
